let SubSwiper = async () => {
    let slider="";
    try {
        let data = await fetch("http://localhost:3001/mortalswiperslide");
        let res = await data.json();
        slider =res.map((item)=>{
            return `<div class="swiper-slide w-[307px] flex flex-col gap-1" id="two">
        ...
        document.querySelector(".swiper-wrapperTwo").insertAdjacentHTML("afterbegin", slider.join(""));
