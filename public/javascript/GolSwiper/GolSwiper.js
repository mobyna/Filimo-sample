let GolSwiper = async () => {
    let slider="";
    try {
        let data = await fetch("http://localhost:3001/golSwiper");
        let res = await data.json();
        
        console.log(res);
        slider =res.map((item)=>{
            return `<div class="swiper-slide w-[307px] flex flex-col gap-1" id="Five">
        ...
        document.querySelector(".swiper-wrapperFive").insertAdjacentHTML("afterbegin", slider.join(""));
